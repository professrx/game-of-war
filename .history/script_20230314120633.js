...
class GameOfWar {
    ...
    war() {
        /// if player one has 3 or less cards, they lose because war cannot commence (same for player two on line 115)
        if (this.p1.length <= 3){
        /// player two receives player ones cards (vice versa on line 117)
            this.p2.push(...this.pile, ...this.p1)
            this.p1.length = 0
        } else if (this.p2.length <= 3) {
        /// player 2 loses (see 110)
            this.p1.push(...this.pile, ...this.p2)
            this.p2.length = 0
        } else {
        /// the actual war function: both players put 3 cards into the war pile
